...
/**
 * 富邦MVP-MVC110003(人工啟用)服務器
 * @author Pamela-Lin(林宜潔)@2024
 * @category 服務類
 */
@Service
public class Mvc110003Serv {

	private static Logger log = LoggerFactory.getLogger(Mvc110003Serv.class);
	...
	public String service(Document doc) {
		...
		// 2. 創建上行電文實體。
		EmailMaster master = new EmailMaster(doc, "110003");	// 交易代號：110003。
		...
		if ((! cancel) && master.invalid110001()) {
			log.warn("check : (110003) argument errors.");
			return this.service.response(doc, valid, business, database, true).asXML();
		}
		valid = true;
				
		// 4. 進行中的申請單作廢。
		this.service.cancel(master.getIdNo(), "110003", master);
		
		// 5. 新增交易紀錄。
		// (1) 主檔紀錄。
		master.setStatus("00");		// "00": 處理中
		master.setTxStatus("15");	// "15": 完成Email驗證
		master.setErrorCode("0001");
