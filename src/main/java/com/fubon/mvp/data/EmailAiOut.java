...
public class EmailAiOut implements Serializable {
	...
	//外撥時間顯示用格式：2026-09-11 14:23:05
	private static final DateTimeFormatter DATETIME_DISPLAY_FORMAT = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
	...
	/**
	 * 取得外撥時間的顯示用字串，格式固定為 "yyyy-MM-dd HH:mm:ss"（例如 2026-09-11 14:23:05）。
	 * 與 dateTime.toString() 的差異：
	 *   - LocalDateTime.toString() 用 'T' 分隔日期時間（例如 2026-09-11T14:23:05）
	 *   - 若剛好整分/整秒，LocalDateTime.toString() 還會省略秒數（例如 14:23）
	 * 此方法固定用空白分隔、且一律補齊秒數，適合直接顯示給使用者看。
	 * @return 格式化後的字串；若 dateTime 為 null 則回傳 null
	 */
	public String getDateTimeYYYYMMDDH24mmss() {
		return this.dateTime == null ? null : this.dateTime.format(DATETIME_DISPLAY_FORMAT);
	}
	...
	public String toString() {
		return "EmailAiOut [channel=" + channel + ", idNo=" + idNo + ", name=" + name + ", purpose=" + purpose
			+ ", phone=" + phone + ", status=" + status + ", retry=" + retry + ", dateTime=" + getDateTimeYYYYMMDDH24mmss()
